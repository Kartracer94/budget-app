  ...
  const handleDrop = useCallback(
    (e: React.DragEvent) => {
      ...
      const files = Array.from(e.dataTransfer.files).filter(
        (f) => f.name.endsWith(".csv") || f.name.endsWith(".tsv") || f.name.endsWith(".txt")
      );
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Drop your bank or credit card CSV exports here. Both tab-delimited and comma-delimited formats are supported.
        </p>
      </div>
      ...
      >
        <CardContent className="flex flex-col items-center justify-center py-16 gap-3">
          ...
          <p className="text-xs text-muted-foreground">
            Supports bank statements and Amex credit card exports
          </p>
          <input
            id="file-input"
            type="file"
